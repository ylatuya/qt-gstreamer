...
void QtVideoSinkTest::glSurfacePainterFormatsTest()
{
    ...
    if (glsl && !haveGlsl) {
        QSKIP_PORT("Skipping because the system does not support GLSL");
    }

    if (!glsl && !haveArbFp) {
        QSKIP_PORT("Skipping because the system does not support ARB Fragment Programs");
    }
...
#endif
            QSKIP_PORT("Skipping because we have no OpenGL support");
    } else {
